...
ssize_t pseudo_write(struct file * file, const char __user * ubuf , size_t usize, loff_t * off)
{
   int ret, wlen;
   char *tbuf;
   if(kfifo_is_full(&(my_priv->kfifo)))
   {
	   printk("Buffer is full\n");
	  return -ENOSPC;
   }
   wlen = usize;
   if(wlen > kfifo_avail(&(my_priv->kfifo)))
	   wlen = kfifo_avail(&(my_priv->kfifo));

   tbuf = kmalloc(wlen,GFP_KERNEL);

   ret  = copy_from_user(tbuf,ubuf,wlen);

   kfifo_in(&(my_priv->kfifo),tbuf,wlen);
   kfree(tbuf);
...
{
	int ret,i=0;
	my_priv = kmalloc(sizeof(PRIV_OBJ),GFP_KERNEL);

	pclass=class_create(THIS_MODULE,"pseudo_class");
	pbuffer= kmalloc(MAX_SIZE, GFP_KERNEL);
	kfifo_init(&(my_priv->kfifo),pbuffer,MAX_SIZE);
	...
	cdev_init(&(my_priv->cdev),&fops);
	kobject_set_name(&((my_priv->cdev).kobj),"pdevice%d",i);
	ret=cdev_add(&(my_priv->cdev),pdevid,1);
	if (ret){

		printk("Psuedo: Failed to register device\n");
		return -EINVAL;
		}
	my_priv->pdev=device_create(pclass,NULL,pdevid,NULL,"psample%d",i);

printk("Successfully registered,major = %d ,minor = %d",MAJOR(pdevid),MINOR(pdevid));
printk("Pseudo Driver Sample STEP 6 ..Welcome\n");
return 0;
}
static void __exit pseudo_exit(void)
{
	unregister_chrdev_region(pdevid,ndevices);
	cdev_del(&(my_priv->cdev));
	device_destroy(pclass,pdevid);
	class_destroy(pclass);
	printk("Pseudo driver sample STEP 6 BYE \n");
	kfifo_free(&(my_priv->kfifo));
}
...
MODULE_DESCRIPTION("Step 6 Pseudo Drivers");
