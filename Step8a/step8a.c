...
static long pseudo_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
		case MY_IOCTL_LEN:
			printk("IOCTL From Kernel--KFIFO Length is %d\n",kfifo_len(&kfifo));
			break;
		case MY_IOCTL_AVAIL:
			printk("IOCTL From Kernel--KFIFO Avail Space is %d\n",kfifo_avail(&kfifo));
			break;
		/*case MY_IOCTL_RESET:
			printk("ioctl--kfifo GOT RESET %d\n",kfifo_reset(&kfifo));
			break;*/
		}
	return 0;
}
...
static int __init pseudo_init(void)
{
	...
	printk("Step8 pseudo driver sample.. welcome\n");
...
MODULE_DESCRIPTION("Step8a character device driver");
